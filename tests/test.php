...
//After every runChecks-passing $warn contains the array: emptyValueWarning
//errors contains the arrays duplicateErrors, folderIsFileErrors, invalidFormatErrors, inCsvNotInDbErrors
$err = array(); $warn = array();
echo "\n\nfolderIsFileErrors";
runChecks('03_folderIsFileErrors.csv', $configlang, $warn, $err, $c);
st::isEmpty('emptyValueWarning', $warn['emptyValueWarning']);
...
st::isEqual('inCsvNotInDbErrors', $err['inCsvNotInDbErrors'][0], 'Key "test.eintrag03.invalidEintrag" is provided in the CSV but not found in the DB.');
...
st::isNotEmpty('emptyValueWarning', $warn['emptyValueWarning']);
st::isEqual('emptyValueWarning-Msg', $warn['emptyValueWarning'][0], 'Key "test.eintrag01" on row 2 for language "it" is empty and will be skipped.');
st::isEmpty('duplicateErrors', $err['duplicateErrors']);
st::isEmpty('folderIsFileErrors', $err['folderIsFileErrors']);
st::isEmpty('invalidFormatErrors', $err['invalidFormatErrors']);
st::isEmpty('inCsvNotInDbErrors', $err['inCsvNotInDbErrors']);
...
st::isEmpty('invalidFormatErrors', $err['invalidFormatErrors']);
st::isNotEmpty('inCsvNotInDbErrors', $err['inCsvNotInDbErrors']);
st::isEqual('inCsvNotInDbErrors-Msg', $err['inCsvNotInDbErrors'][0], 'Key "test.eintragOnlyInCsv" is provided in the CSV but not found in the DB.');
...
function runChecks($csvFile, $configLang, &$warnings, &$errors, $c) {
	...
	foreach($data as $k => $r) {
		...
		v::checkEmptyValuesInData($configLang, $r, $warnEmptyVal);
	...
	$warnings['emptyValueWarning'] = $warnEmptyVal;
}
